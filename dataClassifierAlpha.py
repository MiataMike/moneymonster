...
with open('masterData.csv') as csvfile:
    ...
    for row in reader:
        ...
        outputList = []
        # get tag
        if (math.atanh(row['N_close0']) - math.atanh(row['N_close1']) >= high):
            tag = 0
        elif (math.atanh(row['N_close0']) - math.atanh(row['N_close1']) >= medium) & (math.atanh(row['N_close0']) - math.atanh(row['N_close1']) < high):
            tag = 1
        elif (math.atanh(row['N_close0']) - math.atanh(row['N_close1']) >= .0) & (math.atanh(row['N_close0']) - math.atanh(row['N_close1']) < medium):
            tag = 2
        elif (math.atanh(row['N_close0']) - math.atanh(row['N_close1']) >= -medium) & (math.atanh(row['N_close0']) - math.atanh(row['N_close1']) < .0):
            tag = 3
        elif (math.atanh(row['N_close0']) - math.atanh(row['N_close1']) >= -high):
            tag = 4
        else:
            tag = 5
